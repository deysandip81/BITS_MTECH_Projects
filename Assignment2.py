...
if __name__ == "__main__":
    ...
    A = np.random.uniform(50.500, 10000.9000, size=(100, 100))
    np.round(A,4,A)
    #print("The Generated Coefficient Matrixx is :: \n" + str(A))

    f = np.random.uniform(1000.5000, 10000.9000, size=(100))
    np.round(f,4,f)
    #print("The Generated value Matrixx is :: \n" + str(f))

    print("\n Solving Using Gauss Elimination without pivot: :")
    ct1 = datetime.datetime.now()
    print("Start time for without pivot :-", ct1)

    gauss_without_pivot(A,f,ds)

    ct2 = datetime.datetime.now()
    print("End time for without pivot", ct2)
    ct = ct2 - ct1
    print("Total time taken for Without Pivot ::" + str(ct.total_seconds()))

    print("\nnSolving Using Gauss Elimination with pivot: :")
    ct1 = datetime.datetime.now()
    print("Start time for with pivot :-", ct1)

    gauss_with_pivot(A,f,ds)

    ct2 = datetime.datetime.now()
    print("End time for with pivot", ct2)
    ct = ct2 - ct1
    print("Total time taken for With Pivot ::" + str(ct.total_seconds()))
